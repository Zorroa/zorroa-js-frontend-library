...
export default function Colors() {
  const colors = Object.keys(variables)
    .sort((a, b) => (a > b ? 1 : -1))
    .filter(variable => variable.search('ZORROA_COLOR_') === 0)
    .map(variable => {
      return {
        name: variable,
  ...
  return (
    <div
      ...
      {colors.map(color => {
        const isLightColor =
          Number.parseInt(color.hex.substring(1), 16) /
            (Number.parseInt('FFFFFF', 16) / 2) >
          1.5
        const accentColor = isLightColor
          ? variables.ZORROA_COLOR_BLACK
          : variables.ZORROA_COLOR_WHITE
        const style = {
        ...
        return (
          <div key={color.name} className="Colors__color" style={style}>
            <div>{color.name}</div>
            <div>
              ${color.name
                .split('_')

...
describe('appReducer', () => {
  describe('SIDEBAR', () => {
    it('left sidebar iconified', () => {
      ...
      expect(appReducer({}, action)).toEqual({ leftSidebarIsIconified: true })
    })

    it('right sidebar iconified', () => {
      const action = { type: ICONIFY_RIGHT_SIDEBAR, payload: true }
      expect(appReducer({}, action)).toEqual({ rightSidebarIsIconified: true })
    })
    ...
    it('TOGGLE_COLLAPSIBLE should set collapsibleOpen', () => {
      ...
      expect(appReducer({}, { type: TOGGLE_COLLAPSIBLE, payload })).toEqual({
        collapsibleOpen: { [collapsibleName]: isOpen },
      })
    })
  })
  ...
  describe('FIELDS', () => {
    it('set metadata fields', () => {
      ...
      expect(appReducer({}, action)).toEqual({ metadataFields: fields })
    })
  })

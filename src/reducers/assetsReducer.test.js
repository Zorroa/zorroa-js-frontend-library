...
describe('assetsReducer', () => {
  ...
  it('ASSET_SEARCH returns asset list', () => {
    ...
    expect(
      assetsReducer({ assetsCounter: 0 }, { type: ASSET_SEARCH, payload }),
    ).toEqual(result)
  })
  ...
  it('ASSET_SEARCH for second page concats assets DEBUG', () => {
    ...
    const state1 = assetsReducer(
      { assetsCounter: 0 },
      { type: ASSET_SEARCH, payload: payload1 },
    )
    ...
    expect(
      assetsReducer(state1, { type: ASSET_SEARCH, payload: payload2 }),
    ).toEqual(result)
  })

  it('ASSET_SEARCH_ERROR returns error', () => {
    const a = 'a'
    expect(assetsReducer([], { type: ASSET_SEARCH_ERROR, payload: a })).toEqual(
      { error: a },
    )
  })
  ...
  it('ASSET_FIELDS returns fields and types', () => {
    ...
    expect(assetsReducer([], { type: ASSET_FIELDS, payload: fields })).toEqual({
      fields,
      types,
    })
  })
  ...
  it('ISOLATE_ASSET returns isolated asset id', () => {
    const id = '1234-abcd'
    expect(assetsReducer([], { type: ISOLATE_ASSET, payload: id })).toEqual({
      isolatedId: id,
    })
  })
  ...
  it('SELECT_ASSETS returns a selected asset id', () => {
    ...
    expect(
      assetsReducer(
        { selectionCounter: 0 },
        { type: SELECT_ASSETS, payload: ids },
      ),

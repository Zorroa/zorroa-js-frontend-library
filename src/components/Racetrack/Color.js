...
class Color extends Component {
  ...
  colors2Hash = (colors, hsvHash) => {
    ...
    const makeKernel = (kernelFn, length, center, radius, wrap) => {
      ...
      for (let x = center - radius + dx / 2; x < center + radius; x += dx) {
        ...
        const bucket = wrap
          ? (Math.floor(x) + length) % length
          : clamp(Math.floor(x), 0, length - 1)
        kernel[bucket] += dx * y
      }
      return kernel
    }
    ...
    const make3dKernel = (
      ...
      for (
        let x = centerArray[0] - radiusArray[0] + dr / 2;
        x < centerArray[0] + radiusArray[0];
        x += dr
      ) {
        const xx = kernelFn(x, centerArray[0], radiusArray[0])
        const xbucket = wrapArray[0]
          ? (Math.floor(x) + lengthArray[0]) % lengthArray[0]
          : clamp(Math.floor(x), 0, lengthArray[0] - 1)
        for (
          let y = centerArray[1] - radiusArray[1] + dr / 2;
          y < centerArray[1] + radiusArray[1];
          y += dr
        ) {
          const yy = kernelFn(y, centerArray[1], radiusArray[1])
          const ybucket = wrapArray[1]
            ? (Math.floor(y) + lengthArray[1]) % lengthArray[1]
            : clamp(Math.floor(y), 0, lengthArray[1] - 1)
          for (
            let z = centerArray[2] - radiusArray[2] + dr / 2;
            z < centerArray[2] + radiusArray[2];
            z += dr
          ) {
            const zz = kernelFn(z, centerArray[2], radiusArray[2])
            const zbucket = wrapArray[2]
              ? (Math.floor(z) + lengthArray[2]) % lengthArray[2]
              : clamp(Math.floor(z), 0, lengthArray[2] - 1)
            kernel[
              xbucket * lengthArray[1] * lengthArray[2] +
                ybucket * lengthArray[2] +
                zbucket
            ] +=
              dr3 * (xx * yy * zz)
          }
    ...
    const encodedNormalHistogramToHashString = hexArray => {
      return hexArray
        .map(v => HEX_DIGITS[clamp(Math.floor(16.0 * v), 0, 15)])
        .join('')
    }

    const histogramToHashString = (hist, maxVal) => {
      return encodedNormalHistogramToHashString(
        histogramToEncodedNormalHistogram(hist, maxVal),
      )
    }
    ...
    switch (this.resolveHashType(hsvHash, colors)) {
      ...
      case 'similarity.hue':
        const hueHash = subHash(
          colors,
          normal,
          0,
          HSV_RANGE[0],
          HL,
          remap(colors.length, 1, 5, 1, 0.5),
          true,
        )
        return hueHash
      ...
      case 'similarity.dephsv': {
        const radiusArray = [
          radii[colors.length],
          colors.length <= 2 ? 1 : 0.5,
          colors.length <= 2 ? 1 : 0.5,
        ]
        const depHash = computeDepHsvHash(
          colors,
          normal,
          [6, 3, 3],
          radiusArray,
      ...
      case 'similarity.deprgb_444': {
        ...
        const rgbHash = computeDepRgbHash(
          colors,
          normal,
          [4, 4, 4],
          radiusArray,
  ...
  setColorHSL = (hsl, key) => {
    ...
    let newColors = colors.map((color, i) => {
      return { ...color, ratio: newRatios[i] }
    })
  ...
  keyExists = key =>
    this.state.colors.findIndex(color => key === color.key) > -1
  ...
  renderSwatches = () => {
    ...
    for (let L = sL; L > 0; L -= dL) {
        ...
        const swatch = (
          <div
            className={classnames('Color-swatch', {
              selected,
              lightOverlay: this.HSLLuma(hsl) < LUMA_OVERLAY_THRESHOLD,
            })}
            key={key}
            style={{
      ...
      swatchRows.push(
        <div className="Color-swatch-row" key={`${L}`}>
          {rowSwatches}
        </div>,
      )
    }
    ...
    swatchRows.push(
      <div className="Color-swatch-row" key={'0'}>
        {rowSwatches}
      </div>,
    )
  ...
  resizeColorStart = key => {
    ...
    this.resizer.capture(
      this.resizeColor,
      this.resizeColorStop,
      0,
      0,
      0,
      1 / COLOR_SLIDER_HEIGHT,
    )
  }

  resizeColor = (x, y) => {
    const newRatio1 = Math.min(
      this.resize1max,
      Math.max(this.resize1min, this.resize1start + y),
    )
    const newRatio2 = Math.min(
      this.resize2max,
      Math.max(this.resize2min, this.resize2start - y),
    )
    const { colors } = this.state
    const newColors = [...colors]
    const newResizeColor1 = { ...colors[this.resizeIndex], ratio: newRatio1 }
    const newResizeColor2 = {
      ...colors[this.resizeIndex + 1],
      ratio: newRatio2,
    }
      ...
      <Widget
        ...
        <div className="Color-body">
          <div className="Color-picker">
            <div className="Color-swatches">{this.renderSwatches()}</div>
          ...
          {isDeveloper &&
            isAdvanced && (
              <div className="Color-developer">
                ...
                <span className="Color-developer-title">
                  Developer controls (experimental)
                </span>
                ...
                <div className="Color-hsvHash">
                  ...
                  <div className="Color-hsvHash-controls">
                    ...
                    <select
                      className="Color-hsvHash-select"
                      onChange={this.selectHsvHash}>
                      {HASHES.map(hash => (
                  ...
                  <span className="Color-hsvHash-hash">
                    {this.colors2Hash(colors, this.state.hsvHash)}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="Color-slider-text">
            Slide to adjust color percentages.
          </div>
          ...
          <div className="Color-slider flexCol">
            <div className="Color-slider-bg">
              ...
              <div
                ...
                Select up to 5 colors<br /> to search by color
              </div>
            </div>
            <div className="Color-slider-fg">
              {colors.map((color, i, a) => {
                ...
                return (
                  <div
                    key={key}
                    className={classnames('Color-slider-entry', 'fullWidth', {
                      lightOverlay,
                    })}>
                    <div
                      className="Color-slider-color flexRowCenter"
                      style={{
                        width: '100%',
                        height: `${Math.round(
                          color.ratio * COLOR_SLIDER_HEIGHT - colorHeightAdjust,
                        )}px`,
                      ...
                      <div
                        className="Color-slider-del flexOff flexRowCenter"
                    ...
                    i < a.length - 1 && (
                      <div
                        className="Color-slider-resizer"

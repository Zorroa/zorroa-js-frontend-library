...
const isNumeric = n =>
  (typeof n === 'number' || typeof n === 'string') && !isNaN(n - parseFloat(n))
...
class Range extends Component {
  ...
  syncWithAppState(nextProps) {
    ...
    if (widget && widget.sliver) {
      ...
      if (widget.sliver.filter && widget.sliver.filter.range) {
        ...
        max = range[field].lte
        const usePrefix =
          field && field.length && field === 'source.fileSize' ? 'bin' : null
  ...
  modifySliver = () => {
    ...
    if (
      ...
      return
    const widget = createRangeWidget(
      field,
      'double',
      min,
      max,
  ...
  minStart = event => {
    ...
    if (range) {
      ...
      this.resizer.capture(
        ...
        min,
        0,
        (range.max - range.min) / this.refs.rangeSliderBox.clientWidth,
        0,
      )
    }
  }
  minUpdate = x => {
    ...
    const max = Math.max(
      this.state.max === undefined ? range.max : this.state.max,
      min,
    ) // let this handle push the other one around
  ...
  maxUpdate = x => {
    ...
    const min = Math.min(
      this.state.min === undefined ? range.min : this.state.min,
      max,
    ) // let this handle push the other one around
    // no modifySliver here, we wait for mouse release
    this.setState({ min, max })
  ...
  render() {
    ...
    const range = this.aggRange(field)
    const min =
      this.state.min === undefined ? range && range.min : this.state.min
    const max =
      this.state.max === undefined ? range && range.max : this.state.max
    ...
    const label =
      !isOpen && active
        ? `${this.valToString(min)} → ${this.valToString(max)}`
        : ''
    return (
      <Widget
        className="Range"
        ...
        <div className="Range-body">
          ...
          <div className="Range-slider-clip">
            <div
              ...
              <div className="Range-slider-center">
                <svg
                  ...
                  {renderSlider && (
                    <rect
                      ...
                      width={`${ltPct - gtPct}`}
                      height="1"
                      style={{ fill: '#73b61c', stroke: 'none' }}
                    />
                  )}
                </svg>
                {renderSlider && (
                  <div
                    className="Range-slider-loc Range-slider-loc-min"
                    style={{ right: `${(1 - gtPct) * 100}%` }}>
                    <div
                      className="Range-slider-handle Range-slider-handle-min"
                ...
                {renderSlider && (
                  <div
                    className="Range-slider-loc Range-slider-loc-max"
                    style={{ left: `${ltPct * 100}%` }}>
                    <div
                      className="Range-slider-handle Range-slider-handle-max"
          ...
          <div className="Range-auto-box">
            <div className="Range-auto-label-box Range-auto-label-box-min">
              <div
                className={classnames('Range-auto-label', 'Range-auto-min', {
                  changed: false,
                })}>
            ...
            <div className="Range-auto-label-box Range-auto-label-box-max">
              <div
                className={classnames('Range-auto-label', 'Range-auto-max', {
                  changed: false,
                })}>

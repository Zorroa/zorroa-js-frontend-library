  ...
  render() {
    ...
    Children.forEach(this.props.children, (child, i) => {
      if (child && menuList.length) {
        menuList.push(<div className="DropdownMenu-separator" key={`${i}s`} />)
      }
      if (child)
        menuList.push(
          <li className="DropdownMenu-item" key={`${i}i`}>
            {child}
          </li>,
        )
    })
    ...
    return (
      <div className="DropdownMenu" style={this.props.style}>
        <div className="DropdownMenu-button" onClick={this.show}>
          {this.props.label}
          <i
            className={classnames('DropdownMenu-caret', 'icon-arrow-down', {
              rot180: this.state.isVisible,
            })}
          />
        </div>
        {this.state.isVisible && (
          <ul className="DropdownMenu-list">{menuList}</ul>
        )}
      </div>
    )
  }

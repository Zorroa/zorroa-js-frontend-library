...
const DialogAlert = (props) => {
  ...
  return (
    <Dialog className='DialogAlert'
            closeFn={confirmAction}
            header={(<span className='DialogAlert-header'>{title}</span>)}
            body={(<div className='DialogAlert-body'>{message}</div>)}
            footer={(<div className='DialogAlert-footer'><div className='DialogAlert-dismiss' onClick={_ => confirmAction && confirmAction()}>OK</div></div>)}/>
  )
}

...
} from '../../actions/trashedFolderActions'
...
class Trash extends Component {
  ...
  restoreSelected(trashedFolder, event) {
    this.props.actions.restoreTrashedFolders(
      this.selectedTrashedFolderIds(trashedFolder),
    )
    this.dismissContextMenu(event)
  }

  deleteSelected(trashedFolder, event) {
    this.props.actions.deleteTrashedFolders(
      this.selectedTrashedFolderIds(trashedFolder),
    )
    this.dismissContextMenu(event)
  }
  ...
  selectedTrashedFolderIds(trashedFolder) {
    ...
    selectedFolderIds.forEach(folderId => {
      ...
      const index =
        folderId !== trashedFolder.folderId && trashedFolders
          ? trashedFolders.findIndex(tf => tf.folderId === folderId)
          : -1
      if (index >= 0) {
  ...
  selectFolder(trashedFolder, event) {
    this.props.actions.selectFolderId(
      trashedFolder.folderId,
      event.shiftKey,
      event.metaKey,
      this.props.trashedFolders,
  ...
  constrainContextMenu = ctxMenu => {
    ...
    if (
      ctxMenu &&
      contextMenuPos &&
      contextMenuPos.y + ctxMenu.clientHeight > window.innerHeight
    ) {
      this.setState({
        contextMenuPos: {
          ...contextMenuPos,
    ...
    return (
      <div>
        ...
        <div
          ...
          <div
            onClick={this.restoreSelected.bind(this, trashedFolder)}
          ...
          <div
            onClick={this.deleteSelected.bind(this, trashedFolder)}
            className="Trash-context-item"
            onContextMenu={this.dismissContextMenu}>
  ...
  renderPermission(acl) {
    ...
    const permisionIcon = isPublic(user, user.permissions)
      ? 'icon-public'
      : undefined
      ...
      <div
        ...
        <div className="Trash-header">
          <div className="Trash-toggle" onClick={this.toggleOpen}>
            {hasChildren ? (
              <i className="Trash-toggleArrow icon-triangle-down" />
            ) : null}
          </div>
          <div className="Trash-select">
            ...
            {userSettings.showFolderCounts !== NO_COUNTS && (
              <div className="Trash-header-count">{trashedFolders.length}</div>
            )}
          </div>
        </div>
        {isOpen ? (
          <div className="Trash-body">
            ...
            <div className="Trash-items">
              {trashedFolders.map(trashedFolder => {
                ...
                const icon = isDyHi
                  ? 'icon-foldercog'
                  : trashedFolder.search
                    ? 'icon-collections-smart'
                    : 'icon-collections-simple'
                return (
                  <div
                    key={trashedFolder.id}
                    className={classnames('Trash-item', {
                      isSelected: selectedFolderIds.has(trashedFolder.folderId),
                    })}
                    onClick={this.selectFolder.bind(this, trashedFolder)}
                    onContextMenu={this.showContextMenu.bind(
                      this,
                      trashedFolder,
                    )}>
                    {contextMenuTrashedFolderId === trashedFolder.id
                      ? this.renderContextMenu(trashedFolder)
                      : null}

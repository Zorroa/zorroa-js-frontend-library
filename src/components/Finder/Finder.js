...
export default class Finder extends Component {
  ...
  flatten(parent, depth, files, filter) {
    ...
    if (depth > 0 && (parent.childIds || matches(parent, filter)))
      files.push(parent)
    ...
    const kids = [...parent.childIds]
      .map(id => ({ ...items.get(id), depth }))
      .sort(sorter)
    kids.forEach(kid => {
      this.flatten(kid, depth + 1, files, filter)
    })
  }
  ...
  renderPads(depth) {
    ...
    for (let i = 0; i < depth; ++i)
      pads.push(<div key={i} className="Finder-pad" />)
    return pads
  }

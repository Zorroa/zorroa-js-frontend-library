...
export default class BoxChooser extends Component {
  ...
  render () {
    ...
    const title = (userAccount && userAccount.name ? userAccount.name.display_name + '\'s' : 'Unknown') + ' Box'
    const icon = loading ? <img className="DropboxChooser-icon" src={spin} alt="Loading Box"/> : <div className="DropboxChooser-icon icon-folder-subfolders" title={title}/>
    return (

...
class Explorer extends Component {
  ...
  changeFilterString = event => {
    this.setStatePromise({ filterString: event.target.value }).then(() =>
      this.updateFilteredFields(this.props),
    )
  }

  cancelFilter = event => {
    this.setStatePromise({ filterString: '' }).then(() =>
      this.updateFilteredFields(this.props),
    )
  }
  ...
  isFavorite(fieldTypes, hasChildren, namespace, metadataFields) {
    ...
    if (hasChildren) {
      isFavorite = true
      const children = Object.keys(fieldTypes).filter(
        f => f.startsWith(namespace) && f.charAt(namespace.length) === '.',
      )
  ...
  toggleFavorites = () => {
    ...
    this.props.actions.saveUserSettings(user, settings)
    this.setStatePromise({ showFavorites }).then(() =>
      this.updateFilteredFields(this.props),
    )
  }
  ...
  updateFilteredFields(props) {
    ...
    const modifiedFavorites =
      ...
      JSON.stringify(this.state.metadataFields)
    let modifiedWidgets = false
    this.state.searchedFields &&
      this.state.searchedFields.forEach(field => {
        if (widgets.findIndex(widget => fieldUsedInWidget(field, widget)) < 0)
          modifiedWidgets = true
      })
    let searchedFields = null
    // Filter all fields by string and sort
    const fields = this.state.showFavorites
      ? metadataFields
      : Object.keys(fieldTypes)
    const lcFilterString = this.state.filterString.toLowerCase()
    const filteredFields = fields
      .filter(field => field.toLowerCase().includes(lcFilterString))
      .sort((a, b) =>
        a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }),
      )
    if (
      JSON.stringify(filteredFields) !==
        JSON.stringify(this.state.filteredFields) ||
      JSON.stringify(collapsibleOpen) !==
        JSON.stringify(this.state.collapsibleOpen) ||
      ...
      const addFields = (ancestors, field) => {
        ...
        for (let i = 0; i < parents.length; ++i) {
          ...
          const index = ancestors.findIndex(
            ancestor => ancestor.namespace === namespace,
          )
          ...
          if (index < 0) {
            ...
            const isSearched =
              isLeaf &&
              widgets.findIndex(widget => fieldUsedInWidget(field, widget)) >= 0
            if (isSearched) searchedFields.push(field)
            const widgetType =
              !hasChildren && widgetTypeForField(field, fieldTypes[field])
      ...
      const modifiedAggFields =
        JSON.stringify(aggFields) !== JSON.stringify(this.state.aggFields)
      if (
        modifiedAggFields ||
        modifiedFavorites ||
        modifiedWidgets ||
  ...
  renderPads(depth) {
    ...
    for (let i = 0; i < depth; ++i)
      pads.push(<div key={i} className="Explorer-item-pad" />)
    return pads
  }
  ...
  renderField(
    ...
    return (
      <div
        className={classnames('Explorer-item', 'Explorer-item-' + itemClass, {
          isSelected,
          isLeaf,
          isOpen,
        })}
        ...
        onClick={e =>
          hasChildren
            ? this.toggleCollapsible(namespace, e)
            : this.toggleWidget(field, e)
        }
        onMouseOver={() => this.hover(namespace)}
        onMouseOut={() => this.clearHover(namespace)}>
        {this.renderPads(depth)}
        <div className="Explorer-left">
          <div
            className={classnames('Explorer-item-toggle', { hasChildren })}
            onClick={e => this.toggleCollapsible(namespace, e)}>
            {hasChildren && (
              <div
                className={classnames(
                  'Explorer-toggle-open',
                  `icon-square-${isOpen ? 'minus' : 'plus'}`,
                )}
              />
            )}
          </div>
        ...
        <div className="Explorer-right">
          ...
          <div
            onClick={e => this.favorite(field, namespace, e)}
            className={classnames(
              'Explorer-item-favorite',
              'icon-star-filled',
      ...
      <div className="Explorer">
        <div className="Explorer-header">
            ...
            {filterString &&
              filterString.length && (
                <div
                  ...
                  className="Explorer-cancel-filter icon-cancel-circle"
                />
              )}
          ...
          <div
            ...
            title="Filter to show only favorite fields"
            className={classnames(
              'Explorer-favorites',
        ...
        <div className="Explorer-body">
          <div className="Explorer-scroll">{filteredComponents}</div>
        </div>
      </div>
    )
  }

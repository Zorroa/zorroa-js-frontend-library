...
class Importer extends Component {
  ...
  renderTimeseries() {
    return <div className="ImporterSection Importer-timeseries" />
  }
  ...
  render() {
    ...
    const filteredErrors = Object.keys(errors)
      .filter(error => error.toLowerCase().includes(lcErrorFilter))
      .sort(compareErrors)
    const errorData = filteredErrors.map(exception => ({
      name: exception,
      value: errors[exception].length,
    }))
        ...
        <div className="Importer-body">
          ...
          <ImporterSection name="errors">
            <div className="Importer-error-pie">
              <PieChart
                field="Errors"
                terms={filteredErrors}
                data={errorData}
                activeIndex={undefined}
                COLORS={COLORS}
                animate={true}
                onSelectPieSection={this.selectError}
              />
            </div>
            <div className="Importer-error-list">
              <div className="Importer-controls">
                <div className="Importer-filter">
                  <input
                    className="Importer-filter-input"
                    value={errorFilter}
                    onChange={e =>
                      this.setState({ errorFilter: e.target.value })
                    }
                  />
                  <div className="Importer-filter-icon icon-search" />
                </div>
                <div className="Importer-retry" onClick={this.retry}>
                  <div className="Importer-retry-icon icon-playall" />
                  <div>Retry</div>
                </div>
              </div>
              {filteredErrors.map(exception => (
                <div key={exception} className="Importer-zebra Importer-error">
                  <div
                    ...
                    onClick={_ => this.toggle(exception)}
                  />
            ...
            <div className="Importer-error-description">
              <div className="Importer-error-description-title">
                Description
              </div>
          ...
          <ImporterSection name="pipelines">
            <div className="Importer-processors">
              <div className="Importer-controls">
                <div className="Importer-filter">
                  <input
                    ...
                    onChange={e =>
                      this.setState({ pipelineFilter: e.target.value })
                    }
                  />
                  <div className="Importer-filter-icon icon-search" />
                </div>
                <div
                  className="Importer-add-processor"
              ...
              {filteredPipeline.map(processor => (
                <div
                  key={processor.id}
                  className="Importer-zebra Importer-processor">
                  <div
                    className={classnames(
                      'Importer-opener',
                      'icon-triangle-down',
                    ...
                    onClick={_ => this.toggle(processor.name)}
                  />
                  <div className="Importer-processor-name">
                    {processor.name}
                  </div>
                </div>
              ))}
            ...
            <div className="Importer-processor-args">
              <div className="Importer-processor-args-controls">
                ...
                <div className="Importer-processor-args-save">Save</div>
              </div>
            ...
            <div className="Importer-controls">
              <div className="Importer-filter">
                <input
                  className="Importer-filter-input"
                  value={taskFilter}
                  onChange={e => this.setState({ taskFilter: e.target.value })}
                />
                <div className="Importer-filter-icon icon-search" />
              </div>
              <div
                className="Importer-retry"
                onClick={this.retry}

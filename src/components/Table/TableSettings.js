...
export default class TableSettings extends Component {
  ...
  suggest = (suggestion, lastAction) => {
    ...
    const suggestions = []
    if (allFieldNames && suggestion && suggestion.length && lastAction === 'type') {
  ...
  submitLayoutKey = (event) => {
    ...
    const isEscape = event.keyCode === 27 || event.key === 'Escape' || event.key === 'Esc'
  ...
  render () {
    const { tableLayouts, selectedTableLayoutId, selectTableLayoutFn, layoutActions, user } = this.props
    const { suggestion, suggestions, renameValue, renameLayoutId } = this.state
    const lockedLayouts = new Set()
    tableLayouts.forEach(layout => {
      if (!layout.hasAccess(user, AclEntry.WriteAccess)) lockedLayouts.add(layout.id)
    })
    return (
      <div className="TableSettings">
        <div className="TableSettings-add-column">
          <div className="TableSettings-add-column-search">
            <Suggestions value={suggestion} suggestions={suggestions}
                         placeholder="Lookup Column"
                         button={<div className="TableSettings-add-column-button">Add Column</div>}
                         onChange={this.suggest} onSelect={this.select}/>
          </div>
        </div>
        <div className="TableSettings-layouts">
          ...
          <div className="TableSettings-layout-list">
            { tableLayouts && tableLayouts.length ? tableLayouts.map(layout => (
              <div key={layout.id}
                ...
                { layout.id === renameLayoutId ? (
                  ...
                  <div className="TableSettings-layout-item-label">{layout.name}</div>
                )}
                <div className="TableSettings-layout-edit icon-pencil" onClick={_ => this.rename(layout)}/>
                <div className="TableSettings-layout-item-lock icon-lock4"/>
              </div>
            )) : (
              <div className={classnames('TableSettings-layout-item', {disabled: true})}>
                <div className="TableSettings-layout-item-check icon-check"/>
        ...
        <div className="TableSettings-help" onClick={this.showHelp}>
          ...
          <div className="TableSettings-help-item">Learn more about working with tables</div>
        </div>
      </div>
    )
  }

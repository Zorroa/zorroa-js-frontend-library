...
class Jobs extends Component {
  ...
  sortOrderClassname = field => {
    ...
    const icon =
      field === sortField
        ? `icon-sort-${sortAscending ? 'asc' : 'desc'}`
        : 'icon-sort'
    ...
    return (
      <div className="Jobs-status" style={{ width: '25%', maxWidth: '25%' }}>
          ...
          <div className="Job-counts">
            {job.warningCount() && (
              <div className="Jobs-warning">
                <div className="Jobs-warning-icon icon-warning2" />
                {job.warningCount()}
              </div>
            )}
            {job.errorCount() && (
              <div className="Jobs-error">
                <div className="Jobs-error-icon icon-spam" /> {job.errorCount()}
              </div>
            )}
            {job.successCount() && (
              <div className="Jobs-success">
                <div className="Jobs-success-icon icon-circle-check" />{' '}
                {job.successCount()}
              </div>
            )}
          </div>
        ...
        {job.type === Job.Export &&
          job.state !== Job.Active && (
            <button
              className={classnames('Jobs-download', 'icon-download2', {
                notDownloaded: job.notDownloaded,
              })}
  ...
  render() {
    ...
    const jobs = Object.keys(this.props.jobs)
      ...
      .filter(job => {
        return (
          job.type === jobType &&
          job.name.toLowerCase().includes(lcFilterString)
        )
      })
      .sort(compare)
    return (
      <div className="Jobs">
          ...
          <div className="Jobs-filter-add">
            <div className="Jobs-filter">
              <input
                ...
                placeholder={`Filter ${jobType}s`}
              />
              {filterString &&
                filterString.length && (
                  <div
                    onClick={this.cancelFilter}
                    className="Jobs-cancel-filter icon-cancel-circle"
                  />
                )}
        ...
        <div className="Jobs-scroll">
          <div className="Jobs-header">
            <div
              className="Jobs-header-title"
              style={{ width: '50%', maxWidth: '50%' }}>
              Name
              <div
                className={this.sortOrderClassname(SORT_NAME)}
                onClick={e => this.sortJobs(SORT_NAME, e)}
              />
            </div>
            <div
              className="Jobs-header-title"
              ...
              Date
              <div
                className={this.sortOrderClassname(SORT_DATE)}
                onClick={e => this.sortJobs(SORT_DATE, e)}
              />
            </div>
            <div
              className="Jobs-header-title"
              ...
              Status
              <div
                className={this.sortOrderClassname(SORT_STATUS)}
                onClick={e => this.sortJobs(SORT_STATUS, e)}
              />
            </div>
          </div>
          <div className="Jobs-body">
            {jobs.map(job => (
              <div
                key={job.id}
                className={classnames('Jobs-job', {
                  isSelected: selectedJobIds.has(job.id),
                  isSelectable: isolateJob || selectJob,
                })}

...
export default class Suggestions extends Component {
  ...
  updateValue = value => {
    ...
    this.setStatePromise({ value, selectedIndex: -1, lastAction: 'type' }).then(
      _ => this.props.onChange(this.state.value, this.state.lastAction),
    )
  }
  ...
  onKeyDown = event => {
    switch (event.key) {
      case 'Enter':
        return this.selectCurrentSuggestionOrValue()
      case 'Tab': {
      ...
      case 'ArrowDown':
        return this.goToSuggestion(this.state.selectedIndex + 1)
      default:
        return this.setState({ lastAction: 'type' })
    }
  }
  ...
  goToSuggestion = newIndex => {
    ...
    if (index !== this.state.selectedIndex) {
      this.setStatePromise({ selectedIndex: index, lastAction: 'select' }).then(
        _ => {
  ...
  selectedSuggestionString() {
    ...
    const suffix =
      suggestion && suggestion.text
        ? suggestion.text.slice(cleanValue.length)
        : ''
    return value + suffix
  }
      ...
      <div className="Suggestions-suggestions">
        {suggestions.map((suggestion, index) => (
          <div
            ...
            className={classnames('Suggestions-suggestion', {
  ...
  render() {
    ...
    const showSuggestions =
      ((!query || !query.query || !query.query.length) &&
        value &&
        value.length) ||
      (query && query.query && query.query !== value)

    return (
      <div
        className={'Suggestions' + (className ? ' ' + className : '')}
        style={style}>
        <div className="Suggestions-overlapping-inputs">
          {showSuggestions && (
            <input
              value={this.selectedSuggestionString()}
              type="text"
        ...
        <div
          onClick={_ => onSelect && onSelect(this.state.value)}
          className="Suggestions-search-button">
          {button}
        </div>

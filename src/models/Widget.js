...
export function removeRaw(field) {
  return field && field.replace(/\.raw$/, '')
}
...
export function widgetTypeForField(field, type) {
  ...
  if (
    type === 'string' &&
    parents[0].toLowerCase() === 'similarity' &&
    parents.length === 3
  ) {
...
export function createExistsWidget(
  field,
  fieldType,
  isMissing,
  isEnabled,
...
export function createFacetWidget(
  ...
  const filter =
    terms && terms.length
      ? new AssetFilter({ terms: { [rawField]: terms } })
      : null
...
export function createRangeWidget(
  field,
  fieldType,
  min,
  max,
...
export function createSimilarityWidget(
  field,
  fieldType,
  hashes,
  minScore,
...
export function createFiletypeWidget(
  field,
  fieldType,
  exts,
  isEnabled,
...
export function createColorWidget(
  field,
  fieldType,
  colors,
  isEnabled,
...
export function createCollectionsWidget(field, fieldType, isEnabled, isPinned) {
  field = '_collections'
  return new Widget({
    type: CollectionsWidgetInfo.type,
    field,
    isEnabled,
    isPinned,
  })
...
export function createMultipageWidget(
  field,
  fieldType,
  asset,
  sortByPage,
  ...
  if (filterMultipage !== 'exists' && filterMultipage !== 'missing')
    filterMultipage = 'exists'
  const existsFilter =
    (filterMultipage === 'exists' || filterMultipage === 'missing') &&
    new AssetFilter({ [filterMultipage]: ['media.clip.parent'] })
  const parentId = asset && asset.parentId()
  const filter =
    parentId && parentId.length
      ? new AssetFilter({ terms: { 'media.clip.parent': [parentId] } })
      : existsFilter
  const order = sortByPage ? [{ field: sortField, ascending: true }] : undefined
  const sliver = new AssetSearch({ filter, order })
  return new Widget({
    type: MultipageWidgetInfo.type,
    field,
    sliver,
    isEnabled,
    isPinned,

...
export default class Folder {
  ...
  isSmartCollection() {
    return (
      this.search &&
      (this.search.aggs || !this.search.empty()) &&
      !this.isDyhi()
    )
  }
  ...
  hasAccess(user, access) {
    ...
    for (let permission of user.permissions) {
      if (
        permission.type === Permission.GroupType &&
        permission.name === Permission.Administrator
      )
  ...
  canAddChildFolderIds(draggedFolderIds, folders, user) {
    ...
    if (draggedFolderIds instanceof Set)
      draggedFolderIds = [...draggedFolderIds]
    if (!draggedFolderIds || !draggedFolderIds.length || !folders || !user)
      return false
  ...
  canAddAssetIds(draggedAssetIds, assets, user) {
    ...
    if (!draggedAssetIds || !draggedAssetIds.length || !assets || !user)
      return false

...
export default class AssetSearch {
  ...
  merge(assetSearch) {
    ...
    if (assetSearch.query) {
      this.query = this.query
        ? `(${this.query}) AND (${assetSearch.query})`
        : assetSearch.query
    }
    if (assetSearch.fields) {
      this.fields = this.fields
        ? union([this.fields, assetSearch.fields])
        : [...assetSearch.fields]
    }
        ...
        for (let key in assetSearch.queryFields) {
          if (assetSearch.queryFields.hasOwnProperty(key)) {
            if (this.queryFields && this.queryFields.hasOwnProperty(key)) {
              this.queryFields[key] = union([
                this.queryFields[key],
                assetSearch.queryFields[key],
              ])
            } else {
  ...
  equals(assetSearch, skip) {
    ...
    return (
      JSON.stringify(new AssetSearch(this), replacer) ===
      JSON.stringify(new AssetSearch(assetSearch), replacer)
    )
  }
